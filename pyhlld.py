...
class HlldConnection(object):
    ...
    def __init__(self, server, timeout, attempts=3):
        ...
        self.logger = logging.getLogger("pyhlld.HlldConnection.%s.%d"
                                        % self.server)
    ...
    def send(self, cmd):
        ...
        for attempt in xrange(self.attempts):
            try:
                ...
                self.logger.exception(
                    "Failed to send command to hlld server! Attempt: %d"
                    % attempt)
        ...
        if not sent:
            self.logger.critical(
                "Failed to send command to hlld server after %d attempts!"
                % self.attempts)
    ...
    def readblock(self, start="START", end="END"):
        ...
        first = self.read()
        if first != start:
            raise HlldError("Did not get block start (%s)! Got '%s'!"
                            % (start, first))
        while True:
            line = self.read()
            if line == "":
    ...
    def response_block_to_dict(self):
        ...
        Convert a block output into a dictionary by splitting
        on spaces, and using the first column as the key, and the
...
class HlldClient(object):
    ...
    def __init__(self, server, timeout=None, hash_keys=False):
        """
        Create a new hlld client.

        :Parameters:
            - server : A server string, provided as "host" or "host:port".
            - timeout: (Optional) A socket timeout to use, defaults to
                       no timeout.
            - hash_keys: (Optional) Should keys be hashed before sending
                       to hlld. Defaults to False.
        """
        self.server = server
        self.timeout = timeout
    ...
    def _server_connection(self):
        """Return a connection to the server, tries to cache connections."""
        if self.server_conn:
    ...
    def create_set(self, name, precision=None, eps=None, in_memory=False):
        """
        Create a new set on the hlld server and returns a HlldSet.

        This will return a HlldSet object attached to the set if the set
        already exists.
        ...
        :Parameters:
            ...
            - in_memory (optional) : If True, specified that the set should
    ...
    def __getitem__(self, name):
        """Get a HlldSet object based on the name."""
        conn = self._server_connection()
        return HlldSet(conn, name, self.hash_keys)

    def list_sets(self, prefix=None):
        """
        List all the available sets.

        Optionally takes a prefix to search for.
...
class HlldSet(object):
    ...
    """Provide an interface to a single Hlld set."""

    def __init__(self, conn, name, hash_keys=False):
        """
        Create a new HlldSet object.
    ...
    def bulk(self, keys):
        """Perform a bulk set command, adds multiple keys in the set."""
        command = ("b %s " % self.name) + \
            " ".join([self._get_key(k) for k in keys])
    ...
    def drop(self):
        """
        Delete the set from the server.
    ...
    def close(self):
        """Close the set on the server."""
        resp = self.conn.send_and_receive("close %s" % (self.name))
        if resp != "Done":
            raise HlldError("Got response: %s" % resp)

    def clear(self):
        """Clear the set on the server."""
    ...
    def info(self):
        """Return the info dictionary about the set."""
        self.conn.send("info %s" % (self.name))
        return self.conn.response_block_to_dict()

    def flush(self):
        """Force the set to flush to disk."""
        resp = self.conn.send_and_receive("flush %s" % (self.name))
        if resp != "Done":
            raise HlldError("Got response: %s" % resp)

    def pipeline(self):
        """Create a HlldPipeline for pipelining multiple queries."""
...
class HlldPipeline(object):
    ...
    def __init__(self, conn, name, hash_keys=False):
        """
        Create a new HlldPipeline object.
    ...
    def merge(self, pipeline):
        ...
        Commands from the other pipeline are appended to the commands of
        this pipeline.
        """
    ...
    def execute(self):
        ...
        All commands are sent to the server in the order issued, and
        responses are returned in appropriate order.
        """
        # Send each command
        buf = self.buf
        self.buf = []
